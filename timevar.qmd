...
With $\boldsymbol{\psi} = \{\psi_i\}_{i=0}^N$ and $\boldsymbol{P}=\{P_i\}_{i=0}^N$ we get all basis functions in matrix form:
...
You need only the diagonal matrix $P = ((P_l, P_k))_{k,l=0}^{N,N}$, where $(P_l, P_k)=\frac{2}{2l+1}\delta_{kl}$.

Matrix form

$$
A = ((\psi_j, \psi_i))_{i,j=0}^{N, N} = S P S^T
$$
...
$$
M = ((\psi''_j, \psi_i))_{i,j=0}^{N, N} = S Q S^T
$$
