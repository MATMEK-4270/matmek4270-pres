...
\begin{bmatrix} P_0(x)&  \ldots& P_{N_x}(x)\end{bmatrix} \begin{bmatrix} \hat{u}_{0,0} & \cdots & \hat{u}_{0,N_y} \\
\vdots & \ddots & \vdots \\
...
def eval2D(xi, yi, uhat):
    Px = np.polynomial.legendre.legvander(xi, uhat.shape[0]-1)
    Py = np.polynomial.legendre.legvander(yi, uhat.shape[1]-1)
    return Px @ uhat @ Py.T
