...
![](https://raw.githack.com/MATMEK-4270/matmek4270-pres/main/figures/wavemovie.apng)
...
Just like for the vibration equation there are several options.

:::

## How to implement $\frac{\partial u}{\partial t}(x, 0) = 0$, option 1 {.smaller}
...
Again, if the first and last rows of $D^{(2)}$ are zero, then $u^{n+1}_0 = u^0_0$ and $u^{n+1}_N=u^0_N$. The boundary values remain as initially set at $t=0$.
