...
$$
\frac{\partial u}{\partial t} + \mathcal{L}(u) =  f \quad \text{and} \quad
...
But how can we eliminate the two $\boldsymbol{u}^0$ terms on each side when there is the matrix $\tilde{D}^{(2)}$?
...
All time dependency is in the expansion coefficients $\hat{u}^{n+1}_{j}, \hat{u}^{n}_{j}, \hat{u}^{n-1}_{j}$.
