...
\newcommand{\halfi}{{1/2}}
\newcommand{\uex}{u_{e}}
\newcommand{\half}{{}^1{\mskip -3mu/\mskip -3mu}_2}
:::
...
consists of four steps:
 ...
 2. fulfilling the equation at discrete space/time points,
...
ax.set_xticks(t, [f'$t_{i}$' for i in range(7)]);
ax.set_yticks([]);
```
 ...
 * Store $u^n$, $n=0,1,\ldots,N_t$ in an array $\boldsymbol{u} = \{u^n\}_{n=0}^{N_t}$.
 * Algorithm:
   - initialize $u^0$
...
```{python}
#| echo : true
%timeit -q -o -n 100 f4(N, float(I), A)
```
